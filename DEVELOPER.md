...
- `render_lines(events, as_json=..., detail=...)` — the default non-TTY output and `--json`'s JSONL are the *same* renderer; `as_json` is a boolean on the same consumption loop, not a separate function, since it's the same event stream just formatted differently, and (deliberately) bypasses `--detail` filtering entirely — JSONL always carries every event, full fidelity. Otherwise each event is passed through `_visible_at(event, detail)` before being formatted: `result` always shows; `none` shows nothing else; `minimal` additionally shows summary-shaped events (`snapshot`/`heartbeat`); `normal` additionally shows `pipeline`/`switched` unconditionally (rare/low-noise, unlike `job`) plus `job` transitions that reach a terminal status (`success`/`failed`/`canceled`/`skipped`) — the created→running/running→pending blips that dominate on a large pipeline are the one thing gated by status at this level; `full` shows everything unfiltered. Every `job`/`pipeline`/`switched` line additionally gets a `_rollup_suffix` appended (job counts, failure count, current stage — the same numbers a dedicated `snapshot`/`heartbeat` line carries), so totals stay visible alongside each transition and not only on separate summary lines; the suffix is omitted when `jobs_total` is still `None` (the pre-job-fetch snapshot/pipeline events). `core/attach.py` computes one `ctx` dict per poll tick and reuses it across every job/pipeline/switched event yielded that tick, so a burst of several transitions in one tick would otherwise repeat the identical suffix string on every line; `render_lines` tracks the last-shown suffix (`last_rollup_suffix`) and collapses immediately-adjacent repeats to just the first line of the burst — the summary shows once per poll interval, not once per transition. Prints with `markup=False, highlight=False, soft_wrap=True` — literal `[TAG]` text must never be read as Rich markup, and a long line (a long job name, or a JSONL object) must never be word-wrapped across multiple physical lines.
