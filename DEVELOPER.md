...
Releases are cut by publishing a GitHub release. Creating the tag is what
starts everything: `hatch-vcs` derives the package version from `git describe`,
so the tag *is* the version.

1. Pick the version and tag name. Tags are `v`-prefixed and the remainder must
...
A PyPI upload cannot be replaced once accepted — only yanked. The gate exists
because `hatch-vcs` fails quietly: a checkout that is not exactly on the tag
still builds, but produces a version like `0.3.1.dev5+g1234abc`. Publishing
that would burn a version number that can never be reused.

`scripts/check_release_version.py` compares the tag against the wheel filename,
the sdist filename, and the version recorded in the wheel's `dist-info`
METADATA — the last being what `importlib.metadata`, and therefore
`ddgl --version`, reports. Run it by hand against a local build with:

```bash
uv build
uv run --no-project python scripts/check_release_version.py --tag v0.4.0 dist/
```

### Release notes

Generated by [git-cliff](https://git-cliff.org/) from conventional commit
subjects, configured in `cliff.toml`. Commits are grouped by type, so
`feat:`/`fix:` prefixes are what make a change show up in the right section;
non-conforming subjects still appear, under Miscellaneous.
...
Publishing needs two things that live outside this repository:

- a **PyPI trusted publisher** for project `ddgl`, owner `DataDog`, repository
  `ddgl-cli`, workflow `release.yml`, environment `pypi`;
- a GitHub **environment** named `pypi`.

Trusted publishing is bound to the workflow *filename*. Renaming
`release.yml` breaks publishing until the PyPI configuration is updated to
match.
