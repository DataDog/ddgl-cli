...
def _print_json(payload: dict[str, object]) -> None:
    ...
    `markup`/`highlight` off so Rich doesn't reinterpret braces or colour
    the output, `soft_wrap` on so it stays a single parseable line however
    narrow the terminal — same treatment render/attach.py gives JSONL.
    """
    console.print(
...
def render_nothing_to_retry(selection: RetrySelection, *, as_json: bool = False) -> None:
    ...
    retried" want different advice, which is why RetrySelection keeps the
    pre-gate count.
    """
    if as_json:
        ...
        console.print(
            f"{selection.matched} job(s) matched, none retryable (already"
            " succeeded or still running). Use --force to retry anyway."
        )
    else:
...
def render_retried_pipeline(pipeline: Pipeline, *, as_json: bool = False) -> None:
